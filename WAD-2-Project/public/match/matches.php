...
<div id="side-nav-short" class="closed">
        <ul class="nav flex-column">
            ...
            <li class="nav-item">
                <a class="nav-link active side-icon" aria-current="page" href="../match/matches.php">
                    <span class="material-icons md-24 d-inline-flex justify-content-center align-items-center menu-option"
...
<div id="side-nav" class="opened">
        <ul class="nav flex-column">
            ...
            <li class="nav-item">
                <a class="nav-link active side-icon d-inline-block mx-3 my-2 d-inline-flex align-items-center" aria-current="page" href="../match/matches.php">
                    <span class="material-icons md-24 d-inline-flex justify-content-center align-items-center pe-2">
                        person_add
                    </span>
                    <span class="d-inline-flex justify-content-center align-items-center body-font icon-label">
...
<div id="side-nav-mobile">
    ...
    <div id = "main-content">
        <div class="container">
            <h1 class = 'text-center my-3'>Matches</h1>
            ...
            <div id="search-results-container" class="bg-white my-3 p-3 rounded-3">
                <ul class="nav nav-tabs">
                    <li class="nav-item">
                        <a :class="{'nav-link': true, active: viewMatch}" aria-current="page" href="#" id="nav-match" @click="toggleView('match')">Matches</a>
                    </li>
                    <li class="nav-item" style="color: #379683; border-color: #379683;">
                        <a :class="{'nav-link': true, active: !viewMatch}" href="#" id="nav-request" @click="toggleView('request')">Requests</a>
                    </li>
                </ul>
                ...
                <div v-else class="container-fluid">
                    ...
                    <div class="row">
                        <div class="col-12 col-md-6 col-lg-4 p-0" v-for="connection in connections">
                            <div class="card m-2 connection-card">
<div class="card-body">
                                    <img v-if="connection.image_name != null" :src="'../images/' + connection.image_name" alt="" class="img-thumbnail rounded-3 connection-card-image" />
                                    <img v-else src="../images/default_profile.jpg" alt="" class="img-thumbnail rounded-3 connection-card-image" />
                                    <h5 class="card-title mt-2">{{ connection.name }}</h5>
                                    <h6 class="card-subtitle mb-2 text-muted mt-2">@{{ connection.username }}</h6>
                                    <span class="d-inline-block card-details-border p-1">{{ getGender(connection.gender) }}</span>
                                    <br />
                                    <span class="d-inline-block card-details mt-2">
                                        {{ connection.school }} ({{ connection.course }}): Year {{ connection.year_of_study }}
                                    </span>
                                    <br />
                                    <p class="card-text mt-2 mb-1">
                                        <span class="fw-bold">Interests: </span>
                                        <br />
                                        {{ connection.interest }}
                                    </p>
                                    <p class="mb-0 fw-bold">Skills: </p>
                                    <span class="d-inline-block card-skills me-1 p-1 mt-2" v-for="skill in connection.skills">
                                        {{ skill }}
                                    </span>
                                    <br>
                                    <button v-if="viewMatch" type="button" id="join-comp-btn" class="btn text-white mt-2 me-1" style="background-color:#379683" @click="sendReq" :comp-id="connection.id" data-bs-dismiss="modal">
                                        Send Request
                                    </button>
                                    <button v-else type="button" id="join-comp-btn" class="btn text-white mt-2 me-1" style="background-color:#379683" @click="acceptConn" :comp-id="connection.id" data-bs-dismiss="modal">
                                        Connect
                                    </button>
                                    <a :href="'../user/users_page.php?id=' + connection.id" target="_blank" class="btn text-white mt-2" style="background-color:#7395AE" id="modal_link">
                                        View Profile
                                    </a>
                                </div>
                            </div>
